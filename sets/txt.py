...
Dictionaries and sets aren’t sequence types, unlike lists and tuples, so we can’t access individual items by using indexing.
...
Items are added to a dictionary by assigning to the key. Dictionaries don’t have an append method.
...
There are two ways of removing items from a dictionary: we can use del with the key, but if we try to remove something that doesn’t exist, the program will crash.

The other way is the pop method, which removes a key and returns the value. If the key doesn’t exist, pop will raise an error.
...
We further developed our menu program to add more information, and looked at two options for storing them; tuples and key/value pairs. Writing the code to retrieve the new values gave you the chance to see why you’d use get rather than indexing. This was to help you understand why one data structure works better than another when you have a choice.

We introduced the setdefault method, which returns the value from the dictionary, if the key exists. It’s different from get because it creates a new entry for the key, if the key doesn’t exist. In that case, setdefault assigns a default value for the key, and returning that default value.

Check out the documentation for Python dictionaries, and refer to it when you practise using the various methods.

We gave some simple examples to show the behaviour of dict methods that you haven’t used yet, e.g. fromkeys(), update(), and values().
...
Python’s hashlib module implements a common interface to secure algorithms, and we looked at hashing data to verify that it wasn't changed. We'll revisit this, when we look at storing data in files.
...
Because sets are unordered, we can’t index or slice them. However, they are more efficient than lists or tuples when checking if an item is present – especially with large amounts of data.
